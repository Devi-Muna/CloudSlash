...
func (m Model) viewList() string {
	...
	// Calculate viewport window for pagination.
	start, end := m.calculateWindow(len(nodes))

	// Header
	// Render Header Columns: ID | TYPE | COST | REASON.
	// Let's standardise:
	...
	filterStatus := ""
	if m.SortMode != "" {
		filterStatus += fmt.Sprintf(" [SORT: %s]", m.SortMode)
	}
	if m.FilterMode != "" {
		filterStatus += fmt.Sprintf(" [FILTER: %s]", m.FilterMode)
	}
	if filterStatus != "" {
	...
	for i := start; i < end; i++ {
		...
		// Type shortener
		dispType := node.TypeStr()
		dispType = strings.TrimPrefix(dispType, "AWS::")
		if len(dispType) > 15 {
			dispType = dispType[:15]
		}
