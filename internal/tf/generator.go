...
// GenerateAtomicNuke generates the "Gordian Knot" Safe Deletion Script.
func (g *Generator) GenerateAtomicNuke(path string) error {
	f, err := os.Create(path)
	if err != nil {
		return err
	}
	...
	// 1. Header
	fmt.Fprintf(f, "#!/bin/bash\n")
	fmt.Fprintf(f, "# CloudSlash Atomic Deletion Sequence (%s)\n", version.Current)
	// fmt.Fprintf(f, "# Generated by 'The Gordian Knot' engine.\n") // REMOVED per user request
	...
	if len(wasteNodes) == 0 {
		fmt.Fprintf(f, "echo 'No waste to nuke! System clean.'\n")
		return nil
	}
	...
	// 4. Generate Commands
	fmt.Fprintf(f, "echo 'Initializing Atomic Nuke...'\n")
